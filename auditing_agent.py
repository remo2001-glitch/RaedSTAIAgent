...
class AuditingAgent:
    ...
    def audit_financial(self, content: str, source: str = "signal") -> Tuple[bool, str]:
        ...
        # truncation_root_fix (#316): كان max_length الافتراضي هنا (4096)
        # يقطع محتوى /signal و/analyze الكاملين قبل وصولهما لمنطق التقسيم
        # المتعدد الرسائل في المستدعي (الذي يُرسِل الفائض في رسالة تيليجرام
        # ثانية بدلاً من فقدانه) — نتيجة ذلك: أقسام كاملة (إدارة المخاطر،
        # السيناريوهات) كانت تُفقَد نهائياً وتُستبدَل بـ"⚠️ اقتُطع المحتوى"
        # رغم أن البنية التحتية لإرسالها كاملة عبر رسالتين موجودة أصلاً.
        # رفع الحد هنا يجعله شبكة أمان أخيرة فقط ضد محتوى فاسد فعلاً
        # (ميجابايتات)، لا حد تشغيلي طبيعي لتقرير مالي مفصَّل.
        return self.audit(
            content,
            source=source,
