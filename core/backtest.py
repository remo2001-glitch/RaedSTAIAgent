...
class BacktestEngine:
    ...
    STRATEGIES = {
        "trend_following": {
            "ema_fast": 12, "ema_slow": 26,   # أسرع → إشارات أكثر
            "entry": "ema_cross_bull", "exit": "ema_cross_bear",
            "stop_pct": 0.08, "target_pct": 0.15,  # R/R = 1.87
        },
        "mean_reversion": {
            "rsi_period": 14, "rsi_oversold": 35, "rsi_overbought": 65,
            "entry": "rsi_oversold", "exit": "rsi_mid",
            "stop_pct": 0.05, "target_pct": 0.10,  # R/R = 2.0
        },
        "breakout": {
            "lookback": 14,   # نافذة أقصر → اختراقات أكثر
            "entry": "high_breakout", "exit": "target_or_stop",
            "stop_pct": 0.05, "target_pct": 0.12,  # R/R = 2.4
        },
    }
    ...
    async def run(self, symbol: str, price_data: List[Dict],
        ...
        # استخدام High/Low الحقيقية من Binance إذا متوفرة
        highs      = [d.get("high", d.get("price", d.get("close", 0))) for d in price_data]
        lows       = [d.get("low",  d.get("price", d.get("close", 0))) for d in price_data]
