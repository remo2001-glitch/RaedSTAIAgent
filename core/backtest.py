...
class BacktestEngine:
    ...
    def _compute_metrics(self, symbol: str, trades: List[Dict],
        ...
        # إصلاح #135: استخدام عدد نقاط البيانات الفعلية لتقدير الفترة
        ts_period = (price_data[-1]["timestamp"] - price_data[0]["timestamp"]) / 86400
        # إذا الفرق أقل من نقاط البيانات × 0.8 → نستخدم عدد النقاط
        period_days = ts_period if ts_period > len(price_data) * 0.5 else len(price_data)
