...
class StateManager:
    ...
    def initialize_owner(self):
        """
        يُشغَّل عند بدء تشغيل البوت.
        يحفظ المالك في state.json ليبقى حتى لو حُذف من ENV.
        """
        owner_str = os.environ.get("OWNER_CHAT_ID", "").strip()
        if owner_str and owner_str.isdigit():
            owner_id = int(owner_str)
            user_data = self._get_user(owner_id)
            # حفظ كـ admin في state.json
            if user_data.get("tier") != "admin":
                user_data["tier"]      = "admin"
                user_data["tier_by"]   = "system_init"
                user_data["tier_since"]= time.time()
                self._set_user(owner_id, user_data)
                logger.info(f"✅ المالك {owner_id} مُسجَّل كـ admin")

        # حفظ المستخدمين من PREMIUM_USERS
        prem_str = os.environ.get("PREMIUM_USERS", "").strip()
        if prem_str:
            for uid_str in prem_str.split(","):
                uid_str = uid_str.strip()
                if uid_str.isdigit():
                    uid = int(uid_str)
                    user_data = self._get_user(uid)
                    if not user_data.get("tier") or user_data.get("tier") == "free":
                        user_data["tier"] = "silver"
                        self._set_user(uid, user_data)
                        logger.info(f"✅ PREMIUM_USERS: {uid} → silver")
    ...
    def get_tier(self, user_id: int) -> str:
        ...
        ١. OWNER_CHAT_ID  → admin (يُقرأ من ENV في كل مرة)
        ٢. ADMIN_USERS    → admin
        ٣. bot_state.json → الباقة المحفوظة (تبقى عبر deploys)
        ٤. PREMIUM_USERS  → silver (ENV)
        ٥. افتراضي        → free
        """
        # ١. المالك — يُقرأ مباشرة من ENV في كل مرة لضمان الاستمرارية
        owner_id_str = os.environ.get("OWNER_CHAT_ID", "").strip()
        if owner_id_str and owner_id_str.isdigit():
            if int(owner_id_str) == user_id:
                return "admin"

        # ٢. ADMIN_USERS
        admin_str = os.environ.get("ADMIN_USERS", "").strip()
        if admin_str:
            try:
                admins = {int(x.strip()) for x in admin_str.split(",")
                           if x.strip().isdigit()}
        ...
        # ٣. bot_state.json — يبقى عبر Railway deploys
        user_data = self._get_user(user_id)
        stored    = user_data.get("tier", "")
        if stored in TIERS and stored != "free":
            return stored

        # ٤. PREMIUM_USERS ENV
        premium_str = os.environ.get("PREMIUM_USERS", "").strip()
        if premium_str:
            try:
                prem_ids = {int(x.strip()) for x in premium_str.split(",")
                             if x.strip().isdigit()}
                if user_id in prem_ids:
                    # حفظ في state.json للمرات القادمة
                    if not stored:
                        user_data["tier"] = "silver"
                        self._set_user(user_id, user_data)
                    return stored if stored in TIERS else "silver"
            except Exception:
                pass
    ...
    def set_tier(self, user_id: int, tier: str, by: str = "admin",
        ...
        """
        يضبط باقة المستخدم.
        requester_id: من يطلب التغيير — يجب أن يكون مديراً.
        إذا requester_id=0 → system call (مسموح).
        """
        if tier not in TIERS:
            logger.warning(f"set_tier: باقة غير صالحة: {tier}")
            return False

        # فحص: هل الطالب مدير؟
        if requester_id != 0:
            requester_tier = self.get_tier(requester_id)
            if requester_tier != "admin":
                logger.warning(
                    f"set_tier REJECTED: user {requester_id} ({requester_tier})"
                    f" حاول تغيير باقة {user_id} → {tier}")
                return False

        # لا يمكن تغيير باقة المالك إلا من النظام
        if self._is_owner(user_id) and requester_id != 0:
            logger.warning(f"set_tier: لا يمكن تغيير باقة المالك")
            return False
