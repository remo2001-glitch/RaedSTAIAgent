...
class StateManager:
    ...
    def _init_redis(self):
        """يُهيّئ اتصال Redis."""
        # Railway يُوفّر Redis URL بأسماء مختلفة — نجرب جميعها
        REDIS_ENV_NAMES = [
            "REDIS_URL",
            "REDIS_PRIVATE_URL",
            "REDIS_PUBLIC_URL",
        ...
        for env_name in REDIS_ENV_NAMES:
            ...
            if val:
                redis_url = val
                logger.info(f"Redis URL من: {env_name} = {val[:30]}...")
                break

        if not redis_url:
            available = [k for k in os.environ if "REDIS" in k.upper()]
            logger.warning(f"Redis URL غير موجود — متغيرات REDIS المتاحة: {available}")
            return

        try:
            import redis as redis_lib
            logger.info(f"Redis library: {redis_lib.__version__}")
        except ImportError:
            logger.error("❌ مكتبة redis غير مُثبَّتة — أضف redis>=4.0.0 لـ requirements.txt")
            return

        # نجرب عدة طرق للاتصال
        # طباعة الـ URL المستخدم (بدون كلمة المرور)
        try:
            import urllib.parse as _up
            parsed = _up.urlparse(redis_url)
            safe_url = f"{parsed.scheme}://***@{parsed.hostname}:{parsed.port}"
            logger.info(f"Redis URL: {safe_url}")
        except Exception:
        ...
        for attempt, kwargs in enumerate([
            {"decode_responses": True, "socket_connect_timeout": 10,
             "socket_timeout": 10},
            {"decode_responses": True, "socket_connect_timeout": 15,
             "socket_timeout": 15, "retry_on_timeout": True},
            ...
            try:
                ...
                self._redis.ping()
                self._redis_ok = True
                logger.info(f"✅ Redis متصل (محاولة {attempt}) — البيانات دائمة")
                return
            except Exception as e:
                logger.error(f"Redis محاولة {attempt} فشلت: {type(e).__name__}: {str(e)[:150]}")

        logger.error("❌ فشل الاتصال بـ Redis — fallback للملف المحلي")
    ...
    def initialize_owner(self):
        """يُشغَّل عند startup — يستعيد جميع البيانات من Redis."""
        self._apply_env_vars()
        # مزامنة الملف المحلي مع Redis (إذا متصل)
        if self._redis_ok:
            self._sync_from_redis()
        logger.info(f"✅ StateManager v3 (Redis={'✅' if self._redis_ok else '❌ fallback'})")

    def _sync_from_redis(self):
        """يزامن الملف المحلي مع Redis عند البدء."""
        try:
            ...
            if users:
                self.save()
                logger.info(f"✅ Redis sync: {len(users)} مستخدم")
        except Exception as e:
