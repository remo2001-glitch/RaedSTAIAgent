"""
Raed State Manager v3 - Redis Backend
Persistent storage across Railway deploys.

Storage priority:
1. Redis - permanent across all deploys
2. ENV Variables - OWNER_CHAT_ID, ADMIN_USERS, PREMIUM_USERS
3. Local file - fallback when Redis unavailable
"""
...
class StateManager:
    ...
    def _init_redis(self):
        ...
        logger.info(f"Redis: محاولة الاتصال بـ {redis_url[:30]}...")
        ...
        for attempt, kwargs in enumerate([
            # محاولة ١: الإعدادات العادية
            {"decode_responses": True, "socket_connect_timeout": 10,
             "socket_timeout": 10},
            # محاولة ٢: بدون SSL verify
            {"decode_responses": True, "socket_connect_timeout": 10,
             "ssl_cert_reqs": None},
            # محاولة ٣: أبسط اتصال ممكن
            ...
            try:
                ...
                logger.warning(f"Redis محاولة {attempt}: {type(e).__name__}: {e}")

        logger.error("❌ فشل الاتصال بـ Redis بعد 3 محاولات — fallback للملف")
