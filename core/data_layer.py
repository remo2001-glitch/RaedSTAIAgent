...
class DataLayer:
    ...
    async def check_spot_available(self, symbol: str) -> dict:
        ...
        async def _try_spot(inst_id: str) -> float:
            """TK1b_fix2: تجربة عدة endpoints للأسهم المُرمَّزة في OKX Spot."""
            # المحاولة 1: مع instType=SPOT
            for url in [
                f"https://www.okx.com/api/v5/market/ticker?instId={inst_id}&instType=SPOT",
                f"https://www.okx.com/api/v5/market/ticker?instId={inst_id}",  # بدون instType
            ]:
                try:
                    data = await _fetch(url, timeout=5)
                    ...
                    continue
            # المحاولة 2: OKX instruments endpoint للتحقق من الإدراج
            try:
                instr_url = f"https://www.okx.com/api/v5/public/instruments?instType=SPOT&instId={inst_id}"
                instr_data = await _fetch(instr_url, timeout=5)
                if instr_data and instr_data.get("code") == "0" and instr_data.get("data"):
                    # الأصل مدرج → اجلب السعر بطريقة مختلفة
                    ticker_url = f"https://www.okx.com/api/v5/market/ticker?instId={inst_id}"
                    ticker = await _fetch(ticker_url, timeout=5)
