...
class DataLayer:
    ...
    async def _hist_binance(self, symbol: str, days: int) -> List[Dict]:
        """
        Binance klines — OHLCV حقيقية بالكامل.
        1000 شمعة/طلب → نجمع بطلبات متعددة.
        """
        results = []
        limit   = 1000
        # نحسب عدد الطلبات المطلوبة
        requests_needed = min((days // limit) + 1, 4)   # حد ٤ طلبات = 4000 يوم

        end_time = None  # نبدأ من الأحدث إلى الأقدم
        for _ in range(requests_needed):
                ...
                try:
                    ...
                    if close > 0 and high >= low > 0:
                        results.append({
                            ...
                            "low":       low,
                            "close":     close,
                            "price":     close,    # للتوافق مع backtest
                            "volume":    vol,
                            "source":    "binance",
                        })
                except (ValueError, TypeError, IndexError):
            ...
            # أقدم timestamp في هذا الدفعة → نطلب قبله
            if data:
    ...
    async def _hist_coingecko(self, symbol: str, days: int) -> List[Dict]:
        """CoinGecko fallback — price + volume فقط."""
        cg      = _cg_id(symbol)
        results = []
        ...
        for chunk in chunks:
            ...
            if isinstance(data, dict):
                ...
                for i, item in enumerate(prices):
                    try:
                        price = float(item[1])
                        vol   = float(vols[i][1]) if i < len(vols) else 0.0
                        results.append({
                            "timestamp": float(item[0]) / 1000,
                            "price":     price,
                            "close":     price,
                            "open":      price,
                            "high":      price * 1.005,   # تقدير عند عدم توفر Binance
                            "low":       price * 0.995,
                            "volume":    vol,
                            "source":    "coingecko",
                        })
    ...
    async def get_news(self, currencies: str = "BTC,ETH",
        ...
        logger.info(f"News: {len(items)} خبر مُجمَّع")
        items = items[:limit]
        if items:
    ...
    async def get_onchain(self, protocol: str = "all") -> Dict:
        ...
        if isinstance(protos, list):
            top = sorted(protos, key=lambda x: float(x.get("tvl") or 0),
                         reverse=True)[:10]
            result["protocols"] = [
                {"name":  p.get("name", ""),
