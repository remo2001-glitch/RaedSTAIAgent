...
class MicrostructureLayer:
    ...
    def format_ar(self, profile: LiquidityProfile,
        ...
        if walls is not None and not isinstance(walls, Exception) and not isinstance(walls, bool):
            ...
            # إصلاح #136: دعم/مقاومة من جدران حقيقية فقط (ليست best bid/ask)
            ...
            # جدار حقيقي = يظهر في buy_walls/sell_walls (قيمة > avg × 5)
            real_support    = bw[0]["price"]  if bw  else 0
            real_resistance = sw[0]["price"]  if sw  else 0
            if real_support > 0 or real_resistance > 0:
                net_ar = ("🟢 ضغط شراء" if net > 0.1 else
                          "🔴 ضغط بيع"  if net < -0.1 else "⚪ متوازن")
                lines += ["", "🧱 *مستويات الدعم والمقاومة*"]
                if real_support > 0:
                    lines.append(f"• دعم (جدار شراء):    ${real_support:,.2f}")
                if real_resistance > 0:
                    lines.append(f"• مقاومة (جدار بيع): ${real_resistance:,.2f}")
                lines.append(f"• الضغط:  {net_ar}")
            elif sup > 0 or res > 0:
                # best bid/ask فقط — نسميها بدقة
                lines += ["", "📊 *أفضل أسعار السوق*"]
                if sup > 0: lines.append(f"• أفضل شراء (Bid): ${sup:,.4f}")
                if res > 0: lines.append(f"• أفضل بيع  (Ask): ${res:,.4f}")
        else:
        ...
        if profile.source == "coingecko":
            ...
            lines += ["", f"📡 Order Book حقيقي من {profile.source.upper()}"]
