...
class MicrostructureLayer:
    ...
    async def detect_walls(self, symbol: str,
        ...
        try:
            if self.session:
                url  = (f"https://api.binance.com/api/v3/depth"
                        f"?symbol={symbol.upper()}USDT&limit={depth_limit}")
                async with self.session.get(
                    url, timeout=aiohttp.ClientTimeout(total=5)
                ) as r:
                    if r.status == 200:
            ...
            logger.warning(f"detect_walls ({symbol}): {e}")
    ...
    # 5. تنسيق التقرير
    # ═══════════════════════════════════════════════════════════
    def format_ar(self, profile: LiquidityProfile) -> str:
        if not profile:
        ...
        source_label = {
            "binance":   "Binance Order Book (حقيقي)",
            "coingecko": "CoinGecko (حجم تداول حقيقي)",
        ...
        if profile.bid_depth_usd > 0:
            lines += [
                ...
                f"({'🟢 ضغط شراء' if profile.imbalance > 0.52 else '🔴 ضغط بيع' if profile.imbalance < 0.48 else '⚪ متوازن'})",
            ]
