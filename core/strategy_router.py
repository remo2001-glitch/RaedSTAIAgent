...
class SignalLayer:
    ...
    def _technical_signal(self, candles: List[Dict]) -> Dict:
        ...
        # Volume confirmation
        if vol_ratio > 1.5 and ema_align:   bullish_pts += 1
        elif vol_ratio > 1.5 and not ema_align: bearish_pts += 1
    ...
    def format_ar(self, s: SignalResult) -> str:
        ...
        return (
            ...
            f"RSI: {s.technicals.get('rsi',50):.0f} | "
            f"EMA: {'✅' if s.technicals.get('ema_align') else '❌'} | "
            f"حجم: {s.technicals.get('vol_ratio',1):.1f}x"
        )
