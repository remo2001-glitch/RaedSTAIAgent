        ...
        if extra:
            ...
            for t in extra:
                ...
                _neg_kw = ("hack", "breach", "ransom", "arrest", "scam", "fraud",
                           ...
                           "sue", "lawsuit", "seized", "seize", "penalty", "fine",
                           "banned", "ban", "shutdown", "warning", "violation", "suspicious",
    ...
    async def analyze_chart_image(self, image_data: bytes,
        ...
        try:
            ...
            prompt_text = (
                ...
                " الأقسام المطلوبة (أجب عن كل قسم بالترقيم فقط):"                " 0-نوع السوق: افحص كل نص في الشارت بعناية شديدة."
                " تنبيه مهم: بعض منصات التداول (مثل OKX) تعرض '10x' أو '5x' بجانب 'التداول الفوري' كتنويه للمستخدم عن إمكانية الرافعة في Futures — هذا لا يعني أن الشارت الحالي هو Futures."
                " اكتب 'Futures/Perp' فقط إذا رأيت بوضوح أياً من:"                " (أ) بالعربية: العقود الدائمة، عقود آجلة، سوق مفتوح"                " (ب) بالإنجليزية: Perp أو Perpetual أو Swap أو USDT-SWAP أو Mark Price أو Funding Rate"                " (ج) أي رمز ينتهي بـ USDT مع وجود مؤشر Funding Rate في الشارت"                " إذا لم يوجد أي من هذه المؤشرات → اكتب 'Spot'"                " 1-الاتجاه العام وقوته"                " 2-مستويات الدعم والمقاومة بالأسعار الدقيقة من الشارت"                " 3-النماذج الفنية الظاهرة"                " 4-تحليل الشموع الأخيرة"                " 5-التوصية: شراء/بيع/انتظار مع توضيح Long أم Short في حالة Futures"                " 6-نقطة الدخول المقترحة بالسعر الدقيق"                " 7-وقف الخسارة بالسعر الدقيق"                " 8-هدف الربح الأول والثاني مع حساب R/R: احسب R/R = (هدف - دخول) / (دخول - وقف خسارة) واذكر النسبة صراحةً مثل R/R = 1:2.5"                " 9-مستوى المخاطرة (نسبة مئوية من رأس المال)"                " 10-مؤشر RSI: ابحث عن قيم RSI في أسفل الشارت (قد تكون RSI6 وRSI12 وRSI24)."                " اقرأ الأرقام بدقة واذكرها. لا تكتب 'لا يظهر' إلا إذا لم يكن هناك مؤشر RSI على الإطلاق."                " 11-مؤشر KDJ: ابحث عن قيم K وD وJ في أسفل الشارت."                " اقرأ الأرقام بدقة واذكرها مع تشخيص التشبع. لا تكتب 'لا يظهر' إلا إذا لم يكن هناك مؤشر KDJ على الإطلاق."                " 12-مؤشر MACD: اذكر DIF وDEA وMACD والهيستوجرام إيجابي أم سلبي ومعناه"                " 13-حجم الصفقة المقترح (Position Size): بناءً على R/R ومستوى الخطورة، اقترح نسبة من رأس المال (مثل 5%/10%/20%) مع تبرير موجز."                " 14-Market Phase: حدد مرحلة السوق (صعود/هبوط/تراكم/توزيع/تعزيز) بناءً على الشارت الكامل."            )
            ctx  = ssl.create_default_context()
            loop = asyncio.get_event_loop()
            ...
            for model in VISION_MODELS:
                try:
                    ...
                    import re as _re_qw
                    # T11b_fix v3: إزالة <think> كامل أو غير مُغلَق
                    content = _re_qw.sub(
                        r"<think>.*?</think>", "", content, flags=_re_qw.DOTALL
                    ).strip()
                    # حالة: <think> بدون </think> (رد منقطع من Qwen3)
                    if "<think>" in content:
