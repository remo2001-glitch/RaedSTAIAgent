...
{{
  ...
  "risk_flags": ["<خطر إن وجد>"]
    ...
    async def analyze_chart_image(self, image_data: bytes,
                                   symbol: str = "") -> str:
        """تحليل صورة الشارت باستخدام Groq Vision."""
        if not self.groq_key:
            return "⚠️ Groq API غير مُفعَّل"
        ...
        try:
            ...
            response_text = await loop.run_in_executor(
                None,
                lambda: urllib.request.urlopen(req, context=ctx, timeout=30).read().decode())

            data    = json.loads(response_text)
            content = data["choices"][0]["message"]["content"].strip()
            logger.info(f"Groq Vision: {len(content)} حرف")
            return content

        except urllib.error.HTTPError as e:
            err = e.read().decode()[:200]
            logger.error(f"Groq Vision {e.code}: {err}")
            return f"❌ خطأ في تحليل الصورة ({e.code}): {err[:80]}"
        except Exception as e:
