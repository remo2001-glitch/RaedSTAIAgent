            ...
            async with self.session.post(
                ...
                if r.status == 200:
                    ...
                    if "sentiment" in result and "sentiment_score" in result:
                        ...
                        logger.warning(f"Groq ({model}): JSON ناقص الحقول: {list(result.keys())}")
                elif r.status == 429:
                    logger.warning(f"Groq rate limit ({model})")
                    await asyncio.sleep(3)
                elif r.status == 401:
                    logger.error(f"Groq 401: مفتاح غير صالح أو منتهي — تحقق من GROQ_API_KEY في Railway")
                else:
                    try:
                        err_text = await r.text()
                        logger.error(f"Groq error {r.status} ({model}): {err_text[:300]}")
                    except Exception:
                        logger.error(f"Groq error {r.status} ({model})")
