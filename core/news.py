...
{{
  ...
  "risk_flags": ["<خطر إن وجد>"]
    ...
    async def analyze_symbol(self, symbol: str,
        ...
        # إصلاح prompt: ثلاثة سيناريوهات + منطق RSI صحيح
        _rsi_context = (
            "RSI منخفض جداً يدل على تشبع بيعي (احتمال ارتداد قصير) وليس استمرار هبوط مؤكد."
            if rsi < 25 else
            "RSI مرتفع يدل على تشبع شرائي (احتمال تصحيح)."
            if rsi > 70 else ""
        )
        prompt = (
            ...
            f" {_rsi_context}"
            " قدم تحليلاً نصياً احترافياً باللغة العربية بدون markdown وبدون JSON."
            " اكتب فقط:"
            " 1-الاتجاه العام (SAR وMACD والمتوسطات)."
            " 2-أهم مستويات الدعم والمقاومة القريبة بالأسعار."
            " 3-ثلاثة سيناريوهات مختصرة:"
            "   أ) هابط: كسر X يفتح Y."
            "   ب) ارتداد: ثبات فوق X يستهدف Y."
            "   ج) انعكاس: استعادة X تعني Z."
            " 4-للشورت: رفض سعري من المقاومة مع ضعف زخم."
            "   للـ scalp long: ثبات فوق الدعم مع وقف قريب."
            "   للمحافظ: انتظار إغلاق فوق X أو كسر Y."
            " اكتب مباشرة بدون مقدمة."
        )
