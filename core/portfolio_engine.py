    ...
    def format_ar(self, state: PortfolioState, regime: RegimeResult,
        ...
        if not state.positions:
            ...
            if _high_conf_not_long:
                ...
                # confidence_wording_ambiguity_fix: "ثقة السوق منخفضة" كانت
                # تُقرَأ وكأنها تصف ثقة تشخيص الـRegime نفسه (وهي غالباً
                # مرتفعة، مثال موثَّق فعلياً: 82% صاعد قوي) بينما تعني فعلياً
                # ثقة الدخول الفردية لكل عملة (والتي هي المنخفضة، عادة تحت
                # 55-60%) — تناقض ظاهري مباشر بين النص والرقم المعروض بجانبه
                # في نفس الرسالة. الإصلاح: صياغة صريحة لا تلتبس مع رقم ثقة
                # الـRegime.
                cause = (f"ثقة الدخول الفردية لكل عملة دون الحد المطلوب "
                         f"رغم تشخيص الاتجاه العام ({regime.description_ar})")
            return (
