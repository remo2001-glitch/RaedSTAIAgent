...
from core.user_manager    import user_manager as _user_manager_singleton
...
class RaedEngine:
    ...
    def __init__(self, config: dict):
        ...
        self.user_manager     = _user_manager_singleton
    ...
    async def connect_user_exchange(self, user_id: int, exchange_name: str,
        ...
        if not self.user_manager.can_use_exchange(user_id, ex_lower):
            logger.warning(
                f"User {user_id} لا يملك صلاحية {exchange_name} (يحتاج باقة مدفوعة)")
            return False
        try:
    ...
    async def _run_4h_scan(self, session: str = "", ksa_hour: int = 0) -> str:
        ...
        try:
            ...
            max_coins = 150 if any(
                self.user_manager.is_premium(uid)
