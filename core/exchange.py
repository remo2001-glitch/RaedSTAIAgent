...
class BitgetExchange(BaseExchange):
    ...
    async def get_balance(self, asset: str = "USDT") -> Balance:
        # Bitget V2 — محاولة عدة endpoints
        endpoints = [
        ...
        for endpoint in endpoints:
            try:
                ...
                logger.info(f"Bitget balance {endpoint.split('/')[-1]}: code={code}")
                if code == "00000":
                    items = data.get("data", [])
                    # شكل قائمة
                    if isinstance(items, list):
                        for b in items:
                            ...
                            if coin == asset.upper():
                                free   = float(b.get("available") or b.get("availableAmount") or 0)
                                locked = float(b.get("frozen") or b.get("frozenAmount") or 0)
                                logger.info(f"Bitget {asset}: free={free}, locked={locked}")
                                return Balance(asset, free, locked, free+locked)
                    # شكل dict
                ...
                logger.warning(f"Bitget balance endpoint {endpoint}: {e}")
        logger.warning(f"Bitget: {asset} غير موجود أو رصيد صفر")
        return Balance(asset, 0, 0, 0)
