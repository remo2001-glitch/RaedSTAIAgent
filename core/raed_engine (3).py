...
class RaedEngine:
    ...
    def __init__(self, config: dict):
        ...
        # state manager reference للـ autotrade
        from core.state_manager import state_manager as _sm_ref_init
        self._sm = _sm_ref_init
            ...
            if getattr(self, "auto_trade_enabled", False) and strong_signals:
                ...
                for s in strong_signals[:2]:
                    try:
                        ...
                        _sm_ref = getattr(self, "_sm", None)
                        for _uid in list(self._user_portfolios.keys()):
                            try:
                                if _sm_ref and not _sm_ref.is_autotrade_on(_uid):
                                    continue
                                # تحميل المحفظة من database
                                _wdata = await self.database.get_virtual_wallet(_uid)
                                if not _wdata:
                                    _user = await self.database.get_or_create_user(
                                        _uid, "", "")
                                ...
                                if _result.get("ok"):
                                    ...
                                    await self.database.save_virtual_wallet(
                                        _uid, _vw.to_dict())
