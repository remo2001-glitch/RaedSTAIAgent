...
{{"translations": ["<ترجمة العنوان 1>", "<ترجمة العنوان 2>"]}}"""
    ...
    async def _call_groq_single(self, model: str, prompt: str,
        ...
        try:
            ...
            # think_leak_fix (#314): بعض نماذج fallback (خصوصاً qwen3 التي
            # تفعِّل وضع "التفكير" افتراضياً) تُرجِع كتلة <think>...</think>
            # كاملة تحوي استدلال النموذج الداخلي — بما في ذلك أحياناً إعادة
            # صياغة تعليمات الـ system prompt نفسها — قبل الجواب الفعلي. كان
            # هذا يمر بالكامل للمستخدم كأنه جزء من التحليل (رُصد في /analyze
            # لـXSPY)، وهو تسريب لمحتوى داخلي وليس مجرد خلل جودة. الإصلاح:
            # إزالة أي كتلة <think>...</think> (بأي حالة أحرف) من كل استجابة
            # نصية قبل استخدامها في أي مسار — JSON أو نص حر على حد سواء،
            # لأن بعض النماذج قد تُخرِج <think> قبل الـJSON أيضاً.
            import re as _re_think
            content = _re_think.sub(r"<think>.*?</think>", "", content,
                                     flags=_re_think.IGNORECASE | _re_think.DOTALL).strip()
            # شبكة أمان: إذا انقطعت الاستجابة داخل <think> بلا إغلاق، نحذف
            # كل شيء بعد <think> المفتوحة (أفضل من إرسال تفكير غير مكتمل)
    ...
    def _neutral_analysis(self) -> Dict:
        return {
            ...
            "confidence":      0.3,
            "risk_flags":      [],
            "source":          "groq/" + GROQ_MODEL.split("/")[-1],
        }
    ...
    async def analyze_chart_image(self, image_data: bytes,
        ...
        try:
            ...
            # chart_prompt_v2: هيكل جديد — قرار قابل للتنفيذ مع تحفظ واضح
            prompt_text = (
                f"أنت خبير تحليل فني كريبتو. حلل شارت {sym_text} واكتب بالعربية فقط.\n"
                f"{_px_instruction}"
                "اتبع هذا الهيكل بالضبط:\n\n"
                "0- نوع السوق: Spot أو Futures/Perp\n"
                "1- الاتجاه العام وقوته: صاعد/هابط/جانبي + قوة الاتجاه\n"
                "2- مستويات الدعم والمقاومة: أسعار دقيقة من الشارت\n"
                "3- النماذج الفنية الظاهرة: أنماط شموع أو مخططات\n"
                "4- تحليل الشموع الأخيرة: آخر 3-5 شموع\n"
                "5- التوصية (مشروطة): شراء مشروط/انتظار/بيع مشروط مع السبب\n"
                "   مثال: انتظار — الدخول فقط بعد إغلاق فوق [مستوى] مع تأكيد الحجم\n"
                "6- نقطة الدخول المقترحة بالسعر الدقيق (مشروطة بتأكيد)\n"
                "7- وقف الخسارة بالسعر الدقيق\n"
                "8- هدف الربح الأول والثاني مع حساب R/R\n"
                "9- مستوى المخاطرة (نسبة مئوية من رأس المال)\n"
                "10- مؤشر RSI: القيمة + تفسير (ذروة شراء>70/بيع<30/محايد)\n"
                "11- مؤشر KDJ: القيم أو لا يظهر\n"
                "12- مؤشر MACD: DIF/DEA/Hist أو لا يظهر\n"
                "13- حجم الصفقة المقترح: نسبة أو انتظار التأكيد\n"
                "14- Market Phase: مرحلة السوق الحالية\n"
                "\nقواعد مهمة:\n"
                "- اذكر الأرقام الدقيقة من الشارت دائماً\n"
                "- لا تجزم بالصعود أو الهبوط — استخدم: مشروط، إذا تحقق، محتمل\n"
                "- إذا RSI>70: نبّه على خطر التصحيح قبل التوصية\n"
                "- اذكر سيناريو الاختراق (شروطه) وسيناريو الفشل\n"
                "- لا تستخدم ** أو _ أو # أو markdown\n"
            )
            ctx  = ssl.create_default_context()
            loop = asyncio.get_event_loop()  # TODO: استبدل بـ asyncio.get_running_loop() في Python 3.10+
