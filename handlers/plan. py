...
async def cmd_plan_month(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # crash_scoping_fix: كانت _get_ohlcv_for_sym مُعرَّفة كدالة متداخلة
    # (nested function) داخل "if scan_mode:" فقط، بينما تُستدعى لاحقاً ضمن
    # list comprehensions متداخلة في نفس الكتلة الشرطية — خطأ فعلي موثَّق:
    # "cannot access free variable '_get_ohlcv_for_sym' where it is not
    # associated with a value in enclosing scope". هذا النمط (دالة مُعرَّفة
    # شرطياً + استدعاؤها داخل تعبير list comprehension في نفس السياق) هش
    # وحسّاس لنسخة بايثون (PEP 709 يُضمِّن التعبيرات الشرطية مباشرة في
    # بايثون 3.12+، فيغيّر طريقة حل المتغيرات الحرة من السياق المحيط). بدل
    # تتبع السبب الدقيق لكل حالة، الإصلاح الجذري الآمن: نقل التعريف خارج أي
    # شرط، ليكون معرَّفاً دائماً وبلا قيد بمجرد تنفيذ الدالة، بصرف النظر عن
    # قيمة scan_mode.
    ...
    if scan_mode:
        # plan_ohlcv_fix: helper لجلب OHLCV حسب نوع الأصل (تعريفه انتقل الآن
        # خارج هذا الشرط — راجع التعليق أعلاه)
    ...
    try:
        ...
        if regime.regime in (Regime.BEAR_TREND, Regime.DISTRIBUTION):
            ...
            if _deployed > 0 and _positions:
                ...
                # bull_rsi_label_context_fix: كان يُعاد استخدام _rsi_label
                # هنا — وهو مُصمَّم لسياق مختلف تماماً (تعافي من ذروة بيع في
                # اتجاه هابط، حيث "RSI>30 ✓" يعني ارتداداً إيجابياً). في هذا
                # السياق البديل (اتجاه صاعد لكن لا ثقة كافية فردياً)، إعادة
                # استخدامه أنتج بالضبط نفس خلل rsi_label_inverted_fix (#319)
                # السابق في موضع جديد: "RSI = 67 > 30 ✓" — علامة ✓ إيجابية
                # لرقم يقترب فعلياً من ذروة الشراء، لا معنى منطقياً له هنا
                # (30 عتبة تعافٍ من تشبّع بيعي، غير ذات صلة في سوق صاعد
                # أصلاً). الإصلاح: نص مخصَّص لهذا السياق تحديداً بدل إعادة
                # استخدام متغيّر مصمَّم لحالة معاكسة.
                if _rsi_btc >= 70:
                    _bull_rsi_note = f"RSI = {_rsi_btc:.0f} ذروة شراء — انتظر تصحيح تحت 60"
                elif _rsi_btc >= 60:
                    _bull_rsi_note = f"RSI = {_rsi_btc:.0f} قريب من ذروة الشراء — الثقة الفردية هي العائق الحالي"
                else:
                    _bull_rsi_note = f"RSI = {_rsi_btc:.0f} طبيعي — الثقة الفردية لكل عملة هي العائق الحالي"
                week_plan = [
                    f"• أسبوع 1: احتفظ بالسيولة (${user_portfolio:,.0f}) — لا أصول مستوفية للثقة رغم الاتجاه العام الصاعد ({_bull_rsi_note})",
