...
async def cmd_live(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from core.user_manager import user_manager as _global_um
    um      = getattr(engine, "user_manager", _global_um)
        ...
        if action == "status" or not args:
            ...
            if has_live:
                ...
                lines = [
                    ...
                    "━━━━━━━━━━━━━━━━━━",
                    f"الباقة: {'💎 مدفوع' if profile.is_premium else '🆓 مجاني'}",
                    f"المنصات المتاحة: {', '.join(profile.allowed_exchanges).upper()}",
                ...
                if profile.is_premium:
                    lines += [
                        "أو",
                        "`/live connect binance KEY SECRET`",
                        "`/live connect bybit KEY SECRET`",
                        "`/live connect bitget KEY SECRET PASSPHRASE`",
                        "`/live connect mexc KEY SECRET`",
                    ]
                lines += ["", "لعرض المنصات: /live exchanges"]
            await msg.edit_text("\n".join(lines), parse_mode=ParseMode.MARKDOWN)
        ...
        # ── ربط Exchange ─────────────────────────────────────
            ...
            if len(args) < 4:
                await msg.edit_text(
                    ...
                    "المنصات: okx · binance · bybit · bitget · mexc\n"
            ...
            if ok:
                ...
                await msg.edit_text(
                    ...
                    f"💰 USDT متاح: ${balance.free:,.2f}\n"
                    f"💰 الإجمالي: ${balance.total:,.2f}\n\n"
                    f"/execute ستستخدم التداول الحقيقي\n"
                    f"رائد يختار أفضل منصة للتنفيذ تلقائياً",
...
async def cmd_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id    = update.effective_user.id
    try:
        owner_id = int(engine.config.get("OWNER_CHAT_ID") or 0)
    except (ValueError, TypeError):
        owner_id = 0
    args       = context.args or []
    ...
    # عرض الحالة الشخصية
    if not args:
        profile = engine.user_manager.get(user_id)
        await update.message.reply_text(
            engine.user_manager.format_profile_ar(user_id),
            parse_mode=ParseMode.MARKDOWN)
        return

    # أوامر المدير فقط (إذا owner_id=0 → لا قيد)
    if owner_id != 0 and user_id != owner_id:
        await update.message.reply_text("⚠️ هذا الأمر للمدير فقط"); return
    ...
    try:
        if sub == "add" and len(args) >= 2:
            ...
            engine.user_manager.add_premium(target_id, by=str(user_id), notes=notes)
            await update.message.reply_text(
                f"✅ تم منح باقة مدفوعة للمستخدم {target_id}")

        elif sub == "remove" and len(args) >= 2:
            target_id = int(args[1])
            ok        = engine.user_manager.remove_premium(target_id)
            await update.message.reply_text(
                f"✅ تم إلغاء الباقة للمستخدم {target_id}" if ok
                else f"⚠️ المستخدم {target_id} غير موجود")

        elif sub == "list":
            premium_users = engine.user_manager.list_premium()
            ...
            target_id = int(args[1])
            await update.message.reply_text(
                engine.user_manager.format_profile_ar(target_id),
                parse_mode=ParseMode.MARKDOWN)
        else:
            await update.message.reply_text(
                "⚠️ الخيارات (للمدير):\n"
