...
def _build_professional_block(
    ...
    if _is_x_asset and _vol_ratio_check < 0.3:
        _risk_warnings.append("⚠️ حجم < 0.3x — مؤشرات RSI/EMA غير موثوقة على سيولة منخفضة")
...
async def cmd_quicksignal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        lines = [
            ...
            + (" — Limit Sell (انتظر ارتداداً)" if entry > price * 1.001 and trend_direction == "short" else
               " — Limit Buy (انتظر تراجعاً)" if entry < price * 0.999 and trend_direction == "long" else ""),
            # quicksignal_fix: النسب من Entry وليس من price الحالي
