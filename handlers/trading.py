...
async def callback_execlevg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    بعد الاختيار → عرض تأكيد Real/Virtual مع الرافعة المختارة."""
    query = update.callback_query
    await query.answer()
    data  = query.data
    parts = data.split("_")
    ...
    try:
        leverage  = int(parts[1])          # 1, 2, 3, 5, 10
    except (ValueError, IndexError):
        leverage  = 1  # افتراضي آمن
    ...
    _lev_warn = {
        1:  "🛡️ أكثر أماناً",
        2:  "⚡ محافظ",
        3:  "⚡ معتدل — راقب المخاطرة",
        5:  "🔥 مرتفع — استخدم وقف الخسارة",
        10: "🚨 عالي جداً — خطر تصفية الحساب",
    }.get(leverage, f"{leverage}x — تحقق من المخاطرة")

    kb_mode = InlineKeyboardMarkup([[
        InlineKeyboardButton(
            f"✅ حقيقي ({_ex_name})",
            callback_data=_mkb("real")),
        InlineKeyboardButton(
            "🎮 افتراضي",
            callback_data=_mkb("virtual")),
    ]])
    _dir_ar = "🟢 شراء" if direction in ("buy","long") else "🔴 بيع"
    await query.edit_message_text(
        f"⚡ *تأكيد الصفقة النهائي*\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"🪙 {symbol} | {_dir_ar}\n"
        f"💰 الحجم: ${size_usd:,.2f}\n"
        f"⚙️ الرافعة: {leverage}x — {_lev_warn}\n\n"
        f"اختر نوع التنفيذ:",
        parse_mode="Markdown",
...
async def callback_execmode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for p in parts:
        if p.startswith("lev") and len(p)>3 and p[3:].isdigit():
            leverage_cb = int(p[3:])
    context.user_data["_exec_leverage"] = leverage_cb
...
async def cmd_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # قبول admin من ADMIN_ID أو tier == "admin"
    try:
        from core.config import ADMIN_ID as _ADMIN_ID
        _is_admin = (user_tier == "admin") or (user_id == _ADMIN_ID)
    except Exception:
        _is_admin = (user_tier == "admin")
    if not _is_admin:
    ...
    try:
        if sub == "add" and len(args) >= 2:
            ...
            # /premium set USER_ID TIER — تعيين باقة مباشرة (اختبار)
            target_id  = int(args[1])
            new_tier   = args[2].lower()
            valid_tiers = ("free", "silver", "gold", "diamond")
            if new_tier not in valid_tiers:
                await update.message.reply_text(
                    f"⚠️ الباقات المتاحة: {', '.join(valid_tiers)}")
                return
            ok = _sm.set_tier(target_id, new_tier, by=str(user_id),
                              requester_id=user_id)
            tier_name = TIERS.get(new_tier, {}).get("name", new_tier)
            if ok:
                await update.message.reply_text(
                    f"✅ تم تعيين *{tier_name}* للمستخدم `{target_id}`\n"
                    f"🔄 الباقة الجديدة: {new_tier}",
                    parse_mode="Markdown")
            else:
                await update.message.reply_text("❌ فشل — تحقق من الصلاحيات")

        elif sub == "reset" and len(args) >= 2:
            # /premium reset USER_ID — إعادة لـ free (مستخدم جديد)
            target_id = int(args[1])
            _sm.set_tier(target_id, "free", by=str(user_id),
                         requester_id=user_id)
            await update.message.reply_text(
                f"✅ تم إعادة `{target_id}` لباقة *مجاني* (مستخدم جديد)\n"
                f"🆓 يمكنه الآن اختبار تجربة المستخدم الجديد",
                parse_mode="Markdown")
            ...
            await update.message.reply_text(
                ...
                "/premium set [user_id] [free|silver|gold|diamond]\n"
                "/premium reset [user_id] — إعادة لـ free\n"
