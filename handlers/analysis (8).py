...
def _build_professional_block(
    ...
    if _conf_score < 40:
        ...
        # FIN4: Position Sizing بناءً على ATR الفعلي (risk 1% من رأس المال)
        # position% = risk% / (SL% / price)
        _sl_pct_fin4 = abs(price - (pro_sl if pro_sl > 0 else price * (1 - _eff_atr * 1.2))) / max(price, 1e-9) * 100
        _pos_fin4    = min(5.0, round(1.0 / max(_sl_pct_fin4 / 100, 0.01) * 100, 1))  # max 5%
        _pos_size_rule = f"{max(3, min(5, round(_pos_fin4)))}% — ثقة منخفضة"
        if hasattr(regime, 'action') and regime.action == "trade_normal":
            try:
        ...
        # FIN4b: Position Sizing بناءً على ATR + risk 1.5%
        _sl_pct_norm = abs(price - (pro_sl if pro_sl > 0 else price * (1 - _eff_atr * 1.2))) / max(price, 1e-9) * 100
        _pos_norm    = min(20.0, round(1.5 / max(_sl_pct_norm / 100, 0.01) * 100, 1))
        _pos_size_rule = f"{max(10, min(20, round(_pos_norm)))}% — ثقة متوسطة"
    else:
        _decision_label = "[HIGH] — حجم 25–35%"
        # FIN4c: Position Sizing بناءً على ATR + risk 2%
        _sl_pct_high = abs(price - (pro_sl if pro_sl > 0 else price * (1 - _eff_atr * 1.2))) / max(price, 1e-9) * 100
        _pos_high    = min(35.0, round(2.0 / max(_sl_pct_high / 100, 0.01) * 100, 1))
        _pos_size_rule = f"{max(25, min(35, round(_pos_high)))}% — ثقة عالية"
    ...
    # FIN3: Trailing Stop عند اتجاه صاعد قوي أو ارتداد من قاع
    _use_trailing = (
        (rsi < 30 and not _fin_corrupted) or          # ارتداد من قاع → trailing يحمي الربح
...
def _calc_atr_raw(candles: list, period: int = 14) -> float:
    """FIN1: ATR الفعلي بدون cap — للمنطق المالي (TP/SL/Position Sizing).
    لا يستخدم للعرض — يستخدم فقط لحساب مستويات الدخول والخروج الواقعية.
    """
    ...
    try:
        ...
        atr   = sum(trs[-period:]) / period
        price = float(candles[-1].get("close", 0))
        # لا cap — لكن نُقيَّد بـ 50% للأصول المنهارة تاريخياً (منطق مالي)
        return min((atr / price * 100) if price > 0 else 3.0, 50.0)
    except (ValueError, TypeError, ZeroDivisionError):
        return 3.0
