...
def _calc_adx(candles: list, period: int = 14) -> float:
    ...
    try:
        ...
        pdi14  = sum(pdms[-period:]) / max(atr14, 0.0001) * 100
        ndi14  = sum(ndms[-period:]) / max(atr14, 0.0001) * 100
        dx     = abs(pdi14 - ndi14) / max(pdi14 + ndi14, 0.0001) * 100
        return round(dx, 1)
    except Exception:
        return 0.0
...
def _build_professional_block(
    ...
    if adx > 45:
        reasons.append(f"• ADX = {adx:.0f} → اتجاه {'هابط' if is_bear else 'صاعد'} قوي جداً (خطورة)")
    elif adx > 30:
        reasons.append(f"• ADX = {adx:.0f} → اتجاه متوسط القوة")
    if macd_hist < 0:
...
async def cmd_analyze(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Fibonacci + Professional
        fib_a      = _calc_fibonacci(candles)
        pro_block_a = _build_professional_block(
            symbol, price, signal if hasattr(signal, "confidence") else type("S",(),{"confidence":0.5,"direction":"neutral","technicals":{}})(),
            regime_obj if "regime_obj" in dir() else type("R",(),{"description_ar":regime_desc,"regime":type("X",(),{"value":"unknown"})()})(),
            candles, rsi, _calc_atr(candles), fib_a
        )
