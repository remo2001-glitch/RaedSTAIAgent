...
async def cmd_trade(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # data_layer_fix: get_price هو الاسم الصحيح
        _price_data = await engine.data_layer.get_price(base, quote="USDT", mkttype="spot")
        price = float((_price_data or {}).get("price", 0))
        if not price:
    ...
    try:
        ...
        # rsi_fix: حساب RSI من OHLCV مباشرة
        try:
            _candles_rsi = await engine.data_layer.get_ohlcv(base, "1d", 20)
            if _candles_rsi and len(_candles_rsi) >= 14:
                _closes = [float(c.get("close", 0)) for c in _candles_rsi if c.get("close")]
                # حساب RSI بسيط
                ...
                _avg_g = sum(_gains[-14:]) / 14 or 0.001
                _avg_l = sum(_losses[-14:]) / 14 or 0.001
                rsi = 100 - (100 / (1 + _avg_g / _avg_l))
            else:
                rsi = 50
        except Exception:
            rsi = 50
