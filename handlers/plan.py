...
async def cmd_plan_month(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text(
        ...
        "⏳ قد يستغرق ٢٠-٤٠ ثانية — يُرجى عدم تكرار الأمر"
    )
    try:
        import asyncio
        ...
        # إضافة السعر الحالي والمرجع لكل عملة
        price_lines = []
        btc_ref = f"📊 مرجع السوق: BTC ({regime.description_ar} {regime.confidence:.0%})"
        for c in candidates:
            sym_p = c.get("symbol","")
            price_v = c.get("price", 0)
            dir_ar  = ("🟢 شراء" if c.get("direction") == "long"
                       else "🔴 بيع" if c.get("direction") == "short"
                       else "⚪ انتظار")
            conf    = c.get("confidence", 0)
            if price_v > 0:
                price_lines.append(
                    f"💎 *{sym_p}* — ${price_v:,.2f} | {dir_ar} | ثقة: {conf:.0%}"
                )
        if price_lines:
        ...
        for i, sym in enumerate(symbols):
            ...
            try:
                ...
                lines += [
                    ...
                    f"  📊 مرجع السوق: BTC ({regime.description_ar})",
                    "",
                ]
            except Exception as e:
