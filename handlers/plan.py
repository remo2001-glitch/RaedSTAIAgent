...
def _calc_price_forecast(candles: list, days: int = 30,
    ...
    try:
        ...
        elif rsi < 35: rsi_momentum = 1.15  # ذروة بيع → انتعاش معتدل
        elif rsi < 45: rsi_momentum = 1.05  # قريب ذروة بيع → إيجابي خفيف (#118)
    ...
    try:
        ...
        if regime.regime in (Regime.BEAR_TREND, Regime.DISTRIBUTION):
            ...
            week_plan = [
                ...
                "• أسبوع 2: دخول جزئي (25%) إذا RSI > 35 وFear < 35",  # إصلاح #139: > وليس <
                "• أسبوع 3: مراجعة وتعديل وقف الخسارة",
                "• أسبوع 4: تقييم النتائج وقرار الاستمرار",
            ]

        # تنبؤ 30 يوم لأول عملة (#139)
        try:
            fc_sym = symbols[0] if symbols else "BTC"
            fc_i   = 0
            fc_c   = ohlcv_all[fc_i] if isinstance(ohlcv_all[fc_i], list) else []
            if len(fc_c) >= 30:
                fc30m = _calc_price_forecast(
                    fc_c, days=30,
                    fear_greed=fear_val,
                    btc_dominance=float((regime.metrics or {}).get("btc_dominance", 50) or 50),
                    market_regime=regime.description_ar,
                )
                cand_fc = next((c for c in candidates if c.get("symbol") == fc_sym), None)
                p_fc    = float((cand_fc or {}).get("price", 0) or 0)
                if fc30m and p_fc > 0:
                    lines.append(_format_forecast_ar(fc_sym, p_fc, fc30m, days=30))
        except Exception as _fce:
            logger.debug(f"planmonth forecast: {_fce}")
...
async def cmd_plan_week(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        # إصلاح #138: مسح أوسع بدلاً من BTC & ETH فقط
    ...
    try:
        ...
        # ── 4. تنبؤ 30 يوم لأول عملة في القائمة — إصلاح #139 ──
        try:
            primary_sym = symbols[0]
            primary_idx = 0
            primary_candles = ohlcv_sym[primary_idx] if ohlcv_sym else []
            if len(primary_candles) >= 30:
                fc30 = _calc_price_forecast(
                    primary_candles, days=30,
                    fear_greed=fear_val,
                    btc_dominance=float(getattr(regime, "metrics", {}).get("btc_dominance", 50) or 50),
                    market_regime=regime.description_ar,
                )
                primary_price = float((price_results[0] or {}).get("price", 0) if not isinstance(price_results[0], Exception) else 0)
                if fc30 and primary_price > 0:
                    lines.append(_format_forecast_ar(primary_sym, primary_price, fc30, days=30))
        except Exception as _fc_e:
            logger.debug(f"planweek forecast: {_fc_e}")
