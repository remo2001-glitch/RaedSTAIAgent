...
async def cmd_outlook(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if _groq_key:
            ...
            # outlook_groq_fix: استخدام engine.news_engine كـ HTTP client
            # (نفس الطريقة التي تعمل بها /signal و/analyze)
            try:
                import urllib.request as _ur, ssl as _ssl
                _req_out = _ur.Request(
                ...
                with _ur.urlopen(_req_out, context=_ctx, timeout=25) as _r_out:
                ...
                # محاولة 2: عبر news_engine مباشرة
                try:
                    if hasattr(engine, "news_engine") and hasattr(engine.news_engine, "_groq_chat"):
                        _resp = await engine.news_engine._groq_chat(
                            messages=[
                        ...
                        if _resp and len(_resp) > 30:
                            _outlook_parts.append(_resp)
                except Exception as _oe2:
                    logger.warning(f"market_outlook fallback: {_oe2}")
