...
def _build_professional_block(
    ...
    # signal_logic_fix: سيناريوهات — حساب من price مباشرة (pro_sl/tp1/tp2 غير متاحة هنا)
    _fib_res_nxt = fib.get("nearest_resistance", 0) if isinstance(fib, dict) else 0
    _atr_val = float(tech.get("atr_pct", 2.0) or 2.0)
    _sl_pct_scen  = min(max(_atr_val * 1.5, 2.0), 7.0)   # 1.5x ATR، حد 7%
    _tp1_pct_scen = _sl_pct_scen * 1.5                    # R:R 1.5
    _tp2_pct_scen = _sl_pct_scen * 2.5                    # R:R 2.5
