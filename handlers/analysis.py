...
_NYSE_TOKENS = {
    ...
    "XXLE", "XLE", "XIWM", "IWM", "XDIA", "DIA",
    # أسهم تكنولوجيا
    "XAAPL", "AAPL", "XGOOGL", "GOOGL", "XGOOG", "GOOG",
    "XAMD", "AMD", "XMETA", "META", "XNVDA", "NVDA",
    "XTSLA", "TSLA", "XMSFT", "MSFT", "XAVGO", "AVGO",
    "XAMZN", "AMZN", "XNFLX", "NFLX", "XINTC", "INTC",
    "XCRM", "CRM", "XADBE", "ADBE", "XPYPL", "PYPL",
    # أسهم صحة ومالية
    "XISRG", "ISRG", "XJNJ", "JNJ", "XJPM", "JPM",
    "XGS", "GS", "XBAC", "BAC", "XWMT", "WMT",
    "XXOM", "XOM", "XCVX", "CVX",
    # أسهم أخرى شائعة
    "XCOIN", "COIN", "XHOOD", "HOOD", "XRBLX", "RBLX",
    "XSPY200", "XNDX", "NDX",
}
...
def _get_market_hours_warning(symbol: str, user_tz_offset: int = 3) -> str:
    ...
    if _sym in _NYSE_TOKENS or _is_x_token:
        ...
        if not _nyse_open:
            ...
            if _weekday > 4:
                # عطلة نهاية الأسبوع
                _day_ar = "السبت" if _weekday == 5 else "الأحد"
                return (
                    f"⏰ *تنبيه:* {_sym} — سوق NYSE/NASDAQ مغلق (عطلة نهاية الأسبوع)\n"
                    f"• اليوم: {_day_ar} | الوقت لديك: {_local_time} ({_tz_name})\n"
                    f"• يفتح السوق الإثنين: {_nyse_open_dt.strftime('%I:%M %p')} ({_tz_name})\n"
                    f"• التداول متاح على OKX 24/7 لكن: سيولة أقل + slippage أعلى"
                )
            else:
                return (
                    f"⏰ *تنبيه:* {_sym} خارج ساعات سوق NYSE/NASDAQ\n"
                    f"• الوقت لديك الآن: {_local_time} ({_tz_name})\n"
                    f"• ساعات التداول: "
                    f"{_nyse_open_dt.strftime('%I:%M %p')}-{_nyse_close_dt.strftime('%I:%M %p')} ({_tz_name}) الإثنين-الجمعة\n"
...
def _fmt_fib_lines(fib: dict, price: float) -> list:
    ...
    if ns > 0: lines.append(f"• أقرب دعم فيبو:    {_fmt_price(ns)} (Fib — يحتاج تأكيد سعري)")
    if nr > 0: lines.append(f"• أقرب مقاومة فيبو: {_fmt_price(nr)} (Fib — يحتاج تأكيد سعري)")
    return lines
...
def _build_professional_block(
    ...
    tech      = {**(getattr(signal, "technicals", {}) or {}), **_extra}  # دمج tech_e
    # ema_bearish_global_fix: حساب ema_bearish داخل الدالة من tech أو candles
    try:
    ...
    _scenario_ar_raw = signal.technicals.get("scenario_ar", "") if hasattr(signal, "technicals") else ""
    # scenario_adx_fix: تذبذب جانبي عند ADX<20 + حجم ضعيف
    _adx_val_s = float(tech.get("adx", 0) or 0) if isinstance(tech, dict) else 0
    _vol_val_s = float(tech.get("vol_ratio", 1.0) or 1.0) if isinstance(tech, dict) else 1.0
    if _adx_val_s > 0 and _adx_val_s < 20 and _vol_val_s < 1.0 and "استمرار" in _scenario_ar_raw:
        _scenario_ar = f"➡️ تذبذب جانبي — انتظر إشارة اتجاه واضحة (ADX={_adx_val_s:.0f})"
    else:
        _scenario_ar = _scenario_ar_raw
    # إصلاح #95 (توحيد القاعدة): SL% من السعر الحالي
    # sl_pct_fix: يُحسَب من pro_entry وليس price الحالي
    _sl_base_price = pro_entry if pro_entry and pro_entry > 0 else price
    sl_pct  = abs(_sl_base_price - pro_sl) / max(_sl_base_price, 1e-9) * 100
    ...
    if _vol_ratio_check < 0.3:
        _risk_warnings.append("⚠️ حجم < 0.3x — مؤشرات غير موثوقة، يُنصح بالانتظار")
        # vol_wait_fix: حجم < 0.3x يفرض WAIT بغض النظر عن الإشارة
        _is_low_volume_wait = True
    else:
        _is_low_volume_wait = False
    ...
    # market_phase_consistency_fix: تحذير عند تناقض Phase مع السعر
    if _mp.lower() == "markup" and ema_bearish:
        _mp_ar = "🟢 صعود (Markup) — ⚠️ تناقض: السعر تحت EMA50"
    elif _mp.lower() == "markdown" and not ema_bearish:
        # Markdown مع سعر فوق EMA50 = قراءة متخلفة محتملة
        _mp_ar = "🔴 هبوط (Markdown) — ⚠️ قد تكون بنية قديمة بعد ارتداد حاد"
    ...
    # signal_logic_fix: RSI extreme
    # ذروة البيع = فرصة ارتداد → تأكيد ✓
    # ذروة الشراء = خطر تصحيح → تحذير ⚠️ وليس تأكيداً
    if rsi < 30:
        _conf_flags = list(_conf_flags) + [f"RSI ذروة بيع ({rsi:.0f}) ✓"]
    elif rsi > 70:
        # RSI>70: تحذير فقط — لا يُعدّ تأكيداً للدخول
        pass  # لا نضيفه لـ _conf_flags
    if _bb_pos_v < 0.15:
        _conf_flags = list(_conf_flags) + ["BB تحت الحد السفلي ✓"]
    elif _bb_pos_v > 0.85:
        # bb_fix: BB فوق الحد العلوي = Overbought شديد → تحذير لا تأكيد
        pass  # لا يُضاف لـ _conf_flags
    ...
    if _is_overbought_wait:
        ...
        # low_confirm_fix: [LOW] أيضاً يحتاج تأكيدات
        if _flags_found < 2:
            _decision_label = "[WAIT] — انتظر تأكيد 2/4 مؤشرات ([LOW] بدون تأكيد)"
        ...
        if _flags_found < 2:
            # confirm_size_fix: 0/4 تأكيدات → WAIT + 0% دائماً
        ...
        # [HIGH]: ثقة >= 75 + تأكيدات كافية
        if _flags_found < 2:
            # confirm_size_fix: حتى [HIGH] يحتاج تأكيدات
    ...
    try:
        ...
        _reversal_risk = min(95, max(5,
            ...
            (10 if conf > 0.65 else 0)
        ))
        _conf_3d_line = f"• اتجاه السوق: {_conf_trend}% | جودة الدخول: {_conf_entry}% | خطر التصحيح: {_reversal_risk}%"
    except Exception:
        _conf_3d_line = ""
    ...
    if _tvl > 0 and not _is_perp_asset:
        ...
        # tvl_synthetic_fix: TVL العام غير ذي صلة للأصول المُرمَّزة
        if _is_x_asset:
            deriv_lines.append(f"• TVL DeFi الكلي: ${_tvl/1e9:.1f}B (مرجعي — غير مرتبط مباشرة بـ {symbol})")
        else:
    ...
    # rr_display_fix: عرض R/R دائماً حتى عند عدم التأكيد
    ...
    _atr_position_note = ""
    if pro_entry > 0 and pro_sl > 0 and atr_pct > 0:
        _entry_sl_dist = abs(pro_entry - pro_sl)
        _portfolio = 10000  # افتراضي $10,000
        _risk_pct = 0.5     # 0.5% مخاطرة من رأس المال
        _pos_from_atr = (_portfolio * _risk_pct / 100) / max(_entry_sl_dist, 1e-9)
    ...
    try:
        ...
        _atr_val      = float((tech or {}).get("atr_pct", atr_pct) or atr_pct or 2.0)
        _sl_pct_scen  = min(max(_atr_val * 1.5, 2.0), 7.0)
        _tp1_pct_scen = _sl_pct_scen * 1.5
        _tp2_pct_scen = _sl_pct_scen * 2.5
        _sl_scen      = price * (1 - _sl_pct_scen  / 100)
        _tp1_scen     = price * (1 + _tp1_pct_scen / 100)
        _tp2_scen     = price * (1 + _tp2_pct_scen / 100)
        _ns_scen      = ns if ns and ns > 0 else price * 0.97
        ...
        parts.extend([
            ...
            (f"🔴 هابط: كسر {_fmt_price(_ns_scen)} بإغلاق 4H + حجم بيع مرتفع"
             f" → {_fmt_price(_ns_scen * 0.97)} أكثر أهمية"),
            (f"❌ إلغاء: Entry لا يُفعَّل بمجرد الوصول"
             f" — يشترط شمعة رفض أو استرداد. كسر {_fmt_price(_sl_scen)} = إلغاء"),
            "",
        ])
    except Exception as _scen_err:
        logger.debug(f"scenarios_build: {_scen_err}")
    ...
    for flag in _conf_flags:
        parts.append(f"  ✓ {flag}")
    if not _conf_flags:
        parts.append("  • لا تأكيدات — انتظر 2/4 مؤشرات (حجم + CVD + هيكل + RSI)")
...
async def cmd_signal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # regime_data_window_fix: كان /signal يجلب 365 يوماً دائماً حتى
        # للأصول المُرمَّزة (X-prefix)، بينما /analyze يستخدم 90 يوماً فقط
        # لها تحديداً (RSI_Fib_fix: "X-prefix يستخدم 90 يوم من OKX لا Yahoo
        # طويل المدى" — لتفادي جودة بيانات أضعف من Yahoo على مدى طويل).
        # هذا التباين ينتج مفتاح تخزين مؤقت مختلف تماماً (365 مقابل 90)
        # فلا يتشارك الأمران أي بيانات، وقد يُنتجان قيم EMA/RSI/Regime
        # مختلفة تماماً لنفس الرمز خلال دقائق (موثَّق فعلياً: XSPY أظهر
        ...
        if len(candles) < _min_candles_sig:
            ...
            # XSKHY_fix v3: لا نوقف التحليل — OHLCV متاح 24/7
            # التحذير يظهر فقط عند التنفيذ وليس عند التحليل
            _is_market_closed_sym = symbol.upper() in {"XSKHY", "SKHY"}
            if _is_market_closed_sym:
                # نُضيف تحذير الساعات ونكمل التحليل
                _kse_note = (
                    "\n\n🕐 ملاحظة: XSKHY — السوق الكوري مغلق حالياً\n"
                    "• التحليل الفني متاح 24/7 — لا تنفيذ خارج 00:00-06:30 UTC"
                )
        ...
        # multi_tf_fix: تصنيف متعدد الأطر
        _tf_note = ""
        if "هابط" in _daily_dir and "صاعد" in _4h_dir and rsi >= 70:
            _tf_note = "\n⚠️ Bullish Extension: صاعد قصير الأجل متشبع — خطر تصحيح مرتفع"
        ...
        # signal_scorer: تقييم جودة الإشارة — يجب قبل full_text
        _scorer_header = ""
        _scorer_footer = ""
        if _SCORER_ENABLED:
            try:
...
async def cmd_liquidity(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # order_flow_fix: التفسير يتطابق مع _ofs_label بدقة
        _reco = []
        if _ofs > 65:
            _reco.append("ضغط شراء قوي في الـ Order Book — دعم محتمل قريب")
        elif _ofs < 35:
            _reco.append("ضغط بيع قوي في الـ Order Book — توخَّ الحذر من شراء فوري")
        else:
            _reco.append("تدفق محايد — انتظر اتجاهاً واضحاً")
        if _fund_pct < -0.01:
...
async def cmd_analyze(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args   = context.args or []
    # analyze_crash_fix: فحص args قبل الوصول
    if not args:
        await _get_message(update, context).reply_text(
            "⚠️ يرجى تحديد الرمز\n"
            "مثال: /analyze BTC أو /analyze XSPY"
        )
        return
    raw_arg = args[0].upper()
    ...
    try:
        ...
        # EMA check
        ema50_val = price  # قيمة افتراضية آمنة
        if len(candles) >= 20:
            try:
                closes = [float(c.get("close", 0)) for c in candles if c.get("close")]
                ...
                # GG2: توحيد منطق ema_bearish
                ema_bearish = price < ema50
                # analyze_crash_fix: تعريف ema50_val للاستخدام في header
                ema50_val = ema50
            except Exception:
                pass
        ...
        # نستخدم _display_symbol_an (XSPY) وليس symbol (SPY)
        try:
            ...
            _tz_an2 = 3
        _nyse_warn_an = _get_market_hours_warning(_display_symbol_an, _tz_an2)
        ...
        parts += [
            ...
            # quicksignal_trend_fix: تنبيه إذا "هابط" مع ارتفاع حاد
            f"🌍 السوق: {regime_desc}" + (
                " — ⚠️ ارتداد صاعد قصير الأجل ضمن سياق هابط كلي"
                if ("هابط" in regime_desc or "Markdown" in regime_desc) and change_24h > 5
                else ""
            ),
        ...
        if _SCORER_ENABLED:
            try:
                ...
                _si_an = build_signal_input(
                    ...
                    pro_sl=_an_sl, pro_tp=_an_tp, pro_entry=_an_ent,
                    sl_pct=_an_sl_pct, tp_pct=_an_tp_pct,
                ...
                # analyze_scorer_g2_fix: تصحيح من النص
                if "Entry" in full or "الدخول" in full: _si_an.has_entry = True
                if "وقف الخسارة" in full: _si_an.has_sl = True
                if "إلغاء" in full or "كسر" in full: _si_an.has_invalidation = True
                full = _sc_an["header"] + "\n\n" + full + "\n\n" + _sc_an["footer"]
            except Exception as _sc_ae:
                logger.debug(f"signal_scorer /analyze: {type(_sc_ae).__name__}: {_sc_ae}")
...
async def cmd_chart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # chart_vars_init: تعريف جميع المتغيرات بقيم افتراضية آمنة في البداية
        symbol = ""
        caption = ""
        _chart_price_sym = ""
        _chart_sym_for_warn = ""
        # chart_nyse_fix v5: كشف مبكر من args قبل Qwen3
        _args_sym = ""
        try:
            _args_sym = (context.args[0].upper().strip() if context.args else "")
            if "/" in _args_sym:
                _args_sym = _args_sym.split("/")[0]
        except Exception:
            _args_sym = ""
        _chart_is_futures = False
        _current_px = 0.0
        _pre_market_warn = ""
        ...
        try:
            ...
            _tz_chart = 3
        _pre_market_warn = ""  # يُحسَب بعد chart_sym_fix v2
        ...
        # chart_nyse_fix v5: fallback شامل مع args
        _chart_sym_for_warn = _chart_price_sym or symbol or _cap_sym or _args_sym or ""
...
async def cmd_quicksignal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            if is_perp_stock:
                # regime_data_window_fix: توحيد مع /signal و/analyze (90 يوماً
                # للأصول المُرمَّزة بدل 250) — لضمان مشاركة نفس مفتاح الكاش
                # وتفادي تصنيفات Regime/EMA متضاربة لنفس الرمز بين الأوامر
                price_d, candles, fear, btc_dom = await asyncio.wait_for(
                    asyncio.gather(
                        engine.data_layer.get_price_perp(symbol),
                        engine.data_layer.get_ohlcv_perp(symbol, 90),
                        engine.data_layer.get_fear_greed(),
                        engine.data_layer.get_btc_dominance(),
                        return_exceptions=True
                    ), timeout=30.0
                ...
                price_d, candles, fear, btc_dom = await asyncio.wait_for(
                    asyncio.gather(
                        ...
                        # regime_data_window_fix: 90 بدل 100 — يطابق /signal
                        # و/analyze تماماً (نفس الشرط RSI_Fib_fix: >89 كافٍ)
                        # لمشاركة نفس بيانات الكاش بدل تصنيفات متضاربة
                        engine.data_layer.get_ohlcv(_x_spot_sym, "1d", 90, "USDT",
                ...
                price_d, candles, fear, btc_dom = await asyncio.wait_for(
                    asyncio.gather(
                        ...
                        # regime_data_window_fix: 365 بدل 250 — يطابق /signal
                        # و/analyze لنفس السبب (مشاركة الكاش، تناسق التصنيف)
                        engine.data_layer.get_ohlcv(symbol, "1d", 365, quote,
                            mkttype=_mkt_arg_qs,
        ...
        if price <= 0:
            # XSKHY_fix v3: لا نوقف quicksignal — رسالة موحَّدة
            await msg.edit_text(
        ...
        lines = [
            ...
            "",
            "📍 *مناطق الدخول والخروج*",
            # quicksignal_fix: توضيح أن النسب محسوبة من Entry
            ...
            f"• وقف الخسارة: {_fmt_price(sl, quote)} ({(sl/max(entry,1e-9)-1)*100:+.1f}%)",
        ]
        ...
        # quicksignal_quality_fix: تحذيرات جودة الصفقة
        _qs_warns = []
        if support > 0 and tp2 < support and tp2 > 0:
            _qs_warns.append("⚠️ هدف 2 تحت الدعم — عدواني، يتطلب كسر الدعم")
        # tp1_support_fix: هدف 1 يجب أن يكون قبل الدعم لا بعده
        if support > 0 and tp1 < support and tp1 > 0:
            _qs_warns.append(f"⚠️ هدف 1 تحت الدعم — اجنِ الأرباح عند الدعم ({_fmt_price(support, quote)}) أولاً")
        if sl > 0 and entry > 0:
        ...
        if direction.startswith("🟢") and rsi < 40:
            ...
            _qs_scenarios = [
                # quicksignal_fix: الاختراق يشترط إغلاق + ثبات + إعادة اختبار
                f"🟢 صاعد: إغلاق فوق {_fmt_price(resistance, quote)} + ثبات + إعادة اختبار ناجح → دخول",
                f"⚪ محايد: تذبذب بين الدعم والمقاومة بدون اتجاه واضح",
                f"🔴 هابط: إغلاق تحت {_fmt_price(support, quote)} + حجم بيع مرتفع → لا دخول",
            ]
        if _qs_scenarios:
        ...
        if pair_primary:
            ...
            # quicksignal_fix: تنبيه ETH/BTC - الأداء النسبي هو المقياس الصحيح
            if resolution.quote and resolution.quote.upper() == "BTC":
                lines.insert(-1,
                    f"\n💡 *ملاحظة:* {display_symbol} يقيس أداء {resolution.base} "
                    f"مقارنةً بـ BTC — ارتفاع {resolution.base}/USDT وحده لا يعني قوة الزوج"
                )
        ...
        # auditing_agent: تدقيق /quicksignal قبل الإرسال
        _qs_text = _clean_md("\n".join(lines))
        _qs_approved, _qs_text = audit_content(_qs_text, source="quicksignal")
        if not _qs_approved:
            logger.warning("auditing_agent: /quicksignal مرفوض → fallback")
        await msg.edit_text(_qs_text, parse_mode="Markdown")
