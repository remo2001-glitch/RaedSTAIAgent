...
def _build_professional_block(
    ...
    # confidence_3d_fix: نظام الثقة 3 درجات
    # 1. ثقة الاتجاه: من regime confidence
    _conf_trend = min(100, int((conf * 100) * 1.2)) if conf else _conf_score
    # 2. ثقة الدخول: تنخفض مع RSI>70 أو بُعد السعر عن EMA
    _ema_dist_pct = abs(price - ema50_val) / max(ema50_val, 1) * 100 if ema50_val else 0
    ...
    _reversal_risk = min(95, max(5,
        ...
        (10 if _cvd_risk > 50 else 5 if _cvd_risk > 20 else 0) -
    ...
    parts.extend([
        ...
        f"• اتجاه السوق: {_conf_trend}% | جودة الدخول: {_conf_entry}% | خطر التصحيح: {_reversal_risk}%",
...
async def cmd_quicksignal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        lines = [
            ...
            + (" — Limit Sell (انتظر ارتداداً)" if entry > price * 1.001 and direction.startswith("🔴") else
               " — Limit Buy (انتظر تراجعاً)" if entry < price * 0.999 and direction.startswith("🟢") else ""),
