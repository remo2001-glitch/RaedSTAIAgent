...
def _build_professional_block(
    ...
    # market_phase_consistency_fix: تحذير عند Markup مع EMA هابط
...
async def cmd_analyze(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"build_professional_block: {type(_pb_err).__name__}: {_pb_err}")
            pro_block_a = f"⚠️ تعذّر بناء التحليل التفصيلي: {type(_pb_err).__name__}"
